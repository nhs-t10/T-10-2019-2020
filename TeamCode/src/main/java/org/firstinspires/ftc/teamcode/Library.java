...
public abstract class Library extends OpMode {
	...
	public static void skrtskrt(float lf, float rf, float lb, float rb) {
	...
	public static void omni(float linear, float rotational, float side) {
	/*
	 ...
	 */
		...
		for (int i = 0; i < 4; i++) {
			...
			rotationalComponent[i] = rotationalMultiplier[i] * rotational;
			eastwestComponent[i] = horizontalMultiplier[i] * side;
		}
		...
		skrtskrt(sums[0], sums[1], sums[2], sums[3]);
